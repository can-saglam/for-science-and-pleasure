...
extension String {
    /// At most `limit` unicode scalars — the unit Postgres's `char_length`
    /// counts — without ever splitting a character (a flag or a family
    /// emoji is several scalars but one character; it stays or goes whole).
    func cappedScalars(_ limit: Int) -> String {
        var out = ""
        var used = 0
        for ch in self {
