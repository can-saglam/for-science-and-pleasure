...
if PHASE == "1a":
    ...
    s2, b2 = svc("/rest/v1/digest_schedules?select=minute")
    print("INFO 1a: singleton PATCH ->", s, "(known 1a bug: mirror trigger lacks WHERE; 1b retires the singleton)")
...
s, b = http("/rest/v1/digest_schedules?select=group_id,day_of_week,hour,minute,timezone", jwt=can_jwt)
check("can reads own group's digest_schedules row", s == 200 and len(b) == 1 and b[0]["group_id"] == gid, f"{s} {b}")
s, b = http("/rest/v1/digest_schedules?select=group_id", jwt=str_jwt)
check("stranger reads 0 digest_schedules", s == 200 and b == [], f"{s} {b}")
s, b = http(f"/rest/v1/digest_schedules?group_id=eq.{gid}", "PATCH", {"minute": 30}, jwt=joyce_jwt, prefer="return=representation")
check("joyce edits group's digest schedule", s == 200 and b and b[0]["minute"] == 30, f"{s} {b}")
s, b = http(f"/rest/v1/digest_schedules?group_id=eq.{gid}", "PATCH", {"minute": 45}, jwt=str_jwt, prefer="return=representation")
check("stranger can't edit it", b == [] or s in (401, 403), f"{s} {b}")
http(f"/rest/v1/digest_schedules?group_id=eq.{gid}", "PATCH", {"minute": 0}, jwt=joyce_jwt, prefer="return=minimal")
...
if PHASE == "1b":
    ...
    rows = sql("select column_name from information_schema.columns where table_name='digest_runs' and column_name='group_id'")
    check("1b: digest_runs has group_id", bool(rows), f"{rows}")
    rows = sql("select public.dispatch_weekly_digest() as fired")
    check("1b: dispatcher runs (0 fired outside window)", isinstance(rows, list) and rows and rows[0]["fired"] in (0, 1), f"{rows}")
