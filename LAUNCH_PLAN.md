...
- [x] 5.2b Add by voice (24 Sep, build 81). "Add something to Can We Go" → Siri asks what → a description ("the latest Anish Kapoor show at the Hayward") goes through the parser (6–40 s), an exact duplicate is answered with who saved it, a near-match is named in the question, and the read-back ("… at Hayward Gallery, on until 18 October"; "… which ended on 2 April 2024") is confirmed before it's parked in the inbox like a shared link. Also: Siri knows the app as "Can We Go" and "CanWeGo" (the display name's question mark hid it), more phrasings and period synonyms
- [x] 5.2c One-sentence adds on iOS 27 (24 Sep, build 82). Adopts the Reminders schema's `createReminder` only (`AddSaveByVoiceIntent`): Events and Places are the lists, a save is the item (dates, been as completed, summary as note, category as tag); Siri fills the title from the sentence, so "Add the latest Anish Kapoor exhibition at Hayward Gallery to Can We Go" runs the same lookup, duplicate check, read-back and inbox as 5.2b. The schema's other fields (flag, recurrence, location trigger, section) are accepted and ignored. iOS 26 keeps the two-step version. Not adopted: update/delete (mark been by voice), `system.searchInApp`
- [ ] 5.2 follow-ups: Remind Me intent; try the phrases, the two-step add and the one-sentence add with real Siri on a device
- [x] 5.3 Visual Intelligence (24 Sep). `VisualSaveQuery` (IntentValueQuery on `SemanticContentDescriptor`) reads the frame's text on the device (Vision), matches it against save titles and venues (whole title, or most of its telling words; venue on the image lifts it; done and missed sink), and returns up to five saves with their widget photo, then **Save to Can We Go**, which opens the composer on the picture and parses it like a shared screenshot. "More results" (`semanticContentSearch`, opens the app) runs the Events or Places search on the best match, or the composer when nothing matches. Frames are kept in Caches for a day. The Visual Intelligence module is device-only, so that part sits behind `canImport` and was checked with a Release device build and its intent metadata; reading, matching and all three hand-offs were checked on the simulator with rendered posters (`CWG_VISUAL` hook). Still to try: the real camera and screenshot flow on a device
...
- [x] 5.6 Live Activity on the day, push-started and ended by `send-reminders` (build 83; migrations 0032–0033 and send-reminders deployed 24 Sep). Only on the day itself: "morning of" reminders and hand-picked ones, starting the moment the reminder goes off and running up to 8 hours (the system cap), gone by midnight home time; the earlier reminders stay notifications. Every group member; the reminder push is unchanged (Apple requires an alert on a push start, so it is silent). Looks like the list cards: theme colour, display face, the photo melting in from the right, a thumbnail in the Dynamic Island. No buttons, a tap opens the save. Settings → Reminders → "Live Activity on the day", on by default, per iPhone; iOS's own switch is respected too. No landscape API in the iOS 27 SDK (the system lays it out); `.small` added for Apple Watch / CarPlay. Follow-up: on the phone, check the Lock Screen and Dynamic Island on the next day-of reminder
